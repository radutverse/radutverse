...
const IDP_OWNER = new Map<string, { status: number; body: any; ts: number }>();

export async function POST(request: NextRequest) {
  try {
    const idempotencyKey = request.headers.get("Idempotency-Key") || undefined;

    if (idempotencyKey && IDP_OWNER.has(idempotencyKey)) {
      const cached = IDP_OWNER.get(idempotencyKey)!;
