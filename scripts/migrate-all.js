...
async function setupWorkspace() {
  try {
    ...
    await fs.ensureDir(path.join(rootDir, 'apps', 'web', 'api', '_lib', 'services'));
    await fs.ensureDir(path.join(rootDir, 'apps', 'web', 'api', '_lib', 'middleware'));

    // Create packages directory
    await fs.ensureDir(path.join(rootDir, 'packages', 'shared', 'src', 'types'));
    await fs.ensureDir(path.join(rootDir, 'packages', 'shared', 'src', 'utils'));
    await fs.ensureDir(path.join(rootDir, 'packages', 'shared', 'src', 'constants'));
    await fs.ensureDir(path.join(rootDir, 'packages', 'shared', 'src', 'schemas'));
  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      try {
        ...
        log('', '');
        log('⚠️', `Migration paused at step ${stepNumber}. Fix the issue and run the failed step again.`, COLORS.yellow);
        log('', '');
