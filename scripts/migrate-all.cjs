  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      try {
        ...
        reportContent += `Error: ${error.message}\n\n`;
        reportContent += `To resume, run:\n\`\`\`bash\n${steps[i].command || "node scripts/setup-workspace.cjs"}\n\`\`\`\n`;
