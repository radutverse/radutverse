...
async function testEndpoint(url, description, method = 'GET', expectedStatus = 200) {
  try {
    log('🔍', `Testing ${description}...`);
    const response = await makeRequest(url, method);

    if (response.status === expectedStatus || (expectedStatus === 200 && response.status >= 200 && response.status < 300)) {
      log('✅', `${description} - Status: ${response.status}`);
      successCount++;
      return true;
    } else {
      log('❌', `${description} - Expected status ${expectedStatus}, got ${response.status}`);
      failureCount++;
      return false;
    }
  } catch (error) {
...
async function runVerification() {
  ...
  try {
    ...
    await testEndpoint(`${apiBaseUrl}/api/check-ip-assets`, 'POST /api/check-ip-assets', 'POST', 400);
    ...
    if (failureCount > 0) {
      ...
      log('', '');
      log('⚠️', `Some endpoints are not responding as expected. This may be normal if the server is not running or these endpoints are not implemented.`);
      process.exit(1);
    } else {
      log('', '');
...
if (!process.argv[2]) {
  log('ℹ���', 'Usage: node scripts/verify-deployment.js <base-url>', COLORS.blue);
  log('ℹ️', 'Example: node scripts/verify-deployment.js http://localhost:5173', COLORS.blue);
